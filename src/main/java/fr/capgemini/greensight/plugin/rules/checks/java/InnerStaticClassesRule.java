package fr.capgemini.greensight.plugin.rules.checks.java;
...
@Rule(key = "GS_27")
@SuppressWarnings("java-greensight:GS_27")
public class InnerStaticClassesRule extends BaseTreeVisitor implements JavaFileScanner {

    private JavaFileScannerContext context;
    private final Deque<Symbol> outerClasses = new LinkedList<>();
    private final Deque<Boolean> atLeastOneReference = new LinkedList<>();
    ...
    private static boolean isParameterizedWithTypeVarFromParent(ClassTree tree) {
        ...
        return parameterizedSuperTypes.stream()
                ...
                .anyMatch(c -> c.symbol().isVariableSymbol());
    }
        ...
        if (!atLeastOneReference.isEmpty()) {
            ...
            if (level >= 0) {
                IntStream.range(0, level)
                        .forEach(value -> atLeastOneReference.pop());
    ...
    private int fromInstance(Symbol symbol, Symbol owner) {
        ...
        for (Symbol outerClass : outerClasses) {
            ...
            if (symbol.equals(outerClass)
                    || (ownerType != null && owner.isTypeSymbol() && outerClass.type().isSubtypeOf(ownerType))) {
                return i;
            }
        }
        return -1;
