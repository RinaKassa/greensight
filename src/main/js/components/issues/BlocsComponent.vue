    ...
    <div class="head">
      <div class="title-group">
        ...
        <div
          class="ellispse-card"
          data-tip
          data-for="issues_by_component_tooltip"
          data-iscapture="true"
        >
          i
          <q-tooltip
          ...
          >
            <div class="tooltip-small">
              <div>
                Each block represents a folder of your codebase that contains at
                least one Java file.
              </div>
              <div>
                Blocks also indicate how many issues have been raised, either by
                Java files contained at the root of the folder (a.k.a.
                <strong>&quot;direct issues&quot;</strong>
                ), or by files contained in sub-folders (a.k.a.
                <strong>&quot;indirect issues&quot;</strong>
                ).
              </div>
            </div>
          </q-tooltip>
...
function filterBlocs() {
  ...
  for (let index = 0; index < activeFilters.length; index += 1) {
    ...
    res = res.concat(
      state.blocs.filter((bloc) => bloc.blocType.includes(filter.key)),
    );
  }
...
async function setFiltersCounters() {
  ...
  const filterMedium = state.blocFilter.find((x) =>
    x.key.includes('medium-impact'),
  );
  const filterHigh = state.blocFilter.find((x) =>
    x.key.includes('high-impact'),
  );

  let minorIssues = [];
  let majorIssues = [];
  let criticalIssues = [];
  for (let index = 0; index < state.blocs.length; index += 1) {
    const bloc = state.blocs[index];
  ...
  minorIssues = [
    ...new Map(minorIssues.map((item) => [item.key, item])).values(),
  ];
  majorIssues = [
    ...new Map(majorIssues.map((item) => [item.key, item])).values(),
  ];
  criticalIssues = [
    ...new Map(criticalIssues.map((item) => [item.key, item])).values(),
  ];
