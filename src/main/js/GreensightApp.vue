...
async function initData(repo) {
  ...
  const issues = await findIssues(projectKey.value, repo);

  const rules = await findRulesGreensight(repo);
  const totalLines = await findTotalAppCodeLines(projectKey.value);
  const baseComponent = await findComponent(projectKey.value);
